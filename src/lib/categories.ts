...
// Crazy, isn't it?
// The function is used to extract the values of the categories array
// and return a tuple of the values.
function extractValues<const C extends readonly { value: string }[]>(
  categories: C,
): { [K in keyof C]: C[K] extends { value: infer V } ? V : never } {
  return categories.map(({ value }) => value) as {
    [K in keyof C]: C[K] extends { value: infer V } ? V : never;
  };
}

export const categoryValues = extractValues(categories);
